defmodule ActorNode do
  ...
  def searchNode(node,initNode,destNode,parentPid) do
    GenServer.cast(node,{:searchNode,initNode,destNode,parentPid})
  end

  def handle_cast({:searchNode,initNode,destNode,parentPid},state) do
    ...
    # IO.puts("dfgf #{initNode} #{destNode} #{getHopCount(String.to_atom(initNode),destNode)}")
    if length(match)!=0 do
      level = Enum.at(match,0)
      col = elem(Integer.parse(Enum.at(match,1),16),0)
      nextNode = Enum.at(Enum.at(routingTable,level),col)
      updateHopCount(String.to_atom(initNode),destNode)
      if nextNode == destNode do
        send parentPid, {:taskDone,initNode,destNode}
        Process.sleep(200)
      else
        searchNode(String.to_atom(nextNode),initNode,destNode,parentPid)
      end
    end
    {:noreply,state}
  end
